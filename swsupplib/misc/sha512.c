...
int
sha512_stream (int fd, void *resblock)
{
  ...
  while (1)
    {
	 ...
	 next round of the loop another block can be read.  */
      ...
      while (1)
	{
	  n = uxfio_sfread (fd, buffer + sum, BLOCKSIZE - sum);
          if (n < 0) { return 1; }
...
int
sha384_stream (int fd, void *resblock)
{
  struct sha512_ctx ctx;
  char buffer[BLOCKSIZE + 72];
  size_t sum;
...
/*
 A arbitrary block-of-bytes interface to sha512
*/
int
sha512_block(void * thisisa, void *resblock, unsigned char * iblock, int icount)
{
     #define SHA_MEM_BLOCKSIZE 512 /* 4096 */
  struct sha512_ctx * ctx = (struct sha512_ctx *)thisisa;
  char buffer[SHA_MEM_BLOCKSIZE + 72];
  size_t sum;
  ...
  if (icount == SHA_MEM_BLOCKSIZE) {
      int n;
      sum = 0;

      n = icount;
      sum += n;
      memcpy(buffer, iblock, icount);
      sha512_process_block (buffer, SHA_MEM_BLOCKSIZE, ctx);
      return 0;
  }

  if (icount >= 0 && iblock) {
    if (icount)
    	sha512_process_bytes (iblock, icount, ctx);
    return 0;
  }

  /* icount == 0 && iblock == NULL */
