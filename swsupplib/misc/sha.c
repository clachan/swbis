...
/* WARNING *** This file has been modified.
   Modified by Jim Lowe for inclusion in swbis  2003-03-03
*/
...
/*
 A block-bytes-interface to sha
*/
int
sha_block(void * thisisa, void *resblock, unsigned char * iblock, int icount)
{
     #define SHA_MEM_BLOCKSIZE 512 /* was 4096 */
  struct sha_ctx * ctx = (struct sha_ctx *)thisisa;
  ...
  if (icount == SHA_MEM_BLOCKSIZE) {
      ...
      sha_process_block (buffer, SHA_MEM_BLOCKSIZE, ctx);
      return 0;
  }

  if (icount >= 0 && iblock) {
    if (icount)
    	sha_process_bytes (iblock, icount, ctx);
    return 0;
  }
...
int
sha_stream (int ifd, void *resblock)
{
  /* Important: BLOCKSIZE must be a multiple of 64.  */
#define BLOCKSIZE 512 /* was 4096 */
  struct sha_ctx ctx;
  ...
  while (1)
    {
	 ...
	 next round of the loop another block can be read.  */
      ...
      do
	{
	  n =  uxfio_sfread(ifd, buffer + sum, BLOCKSIZE - sum);
	  sum += n;
	}
