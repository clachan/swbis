...
/* Written by Q. Frank Xia, qx@math.columbia.edu.
   ...
   Modified on 10 OCT 1998  by Jim Lowe for inclusion in a library and to use 
   unix-like file descriptor object.  THESE CHANGES ARE UNRELATED TO THE FSF.
   Please obtain a pristine copy of this file if integrating with a new work.
  ...
  /* Modified by jhl <jhlowe@acm.org> 29NOV99 for inclusion in swtools; 
   *  and, made the test crctab program write the chksum for testing purposes. 
   *     To make the test program: 
   *       if in the swtools tree, 
   *         cc -D CRCTAB  -I.. -I../../include cksum.c
   */
...
main ()
{
  ...
  printf ("the cksum is %u\n", swlib_cksum(STDIN_FILENO));
...
unsigned long
swlib_bsd_sum_from_mem(unsigned char * data, size_t len)
{
	unsigned long sum = 0;
	unsigned char * s;
	int count = 0;
...
swlib_cksum (int uxfio_fd)
{
  ...
  while ((bytes_read = uxfio_read (uxfio_fd, buf, BUFLEN)) > 0)
    {
