 ...
 *
 *  Modified 2003-11-06  by Jim Lowe for inclusion in swbis
 *
                    ...
                    const struct option *longopts)
{
	...
	while ((opt = (alternative?
	              ugetopt_long_only(argc,argv,optstr,longopts,&longindex):
	              ugetopt_long(argc,argv,optstr,longopts,&longindex))) 
               != EOF) 
		if (opt == '?' || opt == ':' )
			exit_code = 1;
...
int main(int argc, char *argv[])
{
	...
	while ((opt=ugetopt_long(argc,argv,shortopts,longopts,NULL)) != EOF) 
		switch (opt) {
		case 'a':
			alternative=1;
			break;
